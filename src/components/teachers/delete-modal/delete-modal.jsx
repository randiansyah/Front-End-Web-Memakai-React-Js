...
import { deleteTeacher } from "../../../redux/actions/teachers/teachersActions";
import Swal from "sweetalert2";

const DeleteModalComponent = ({ ...props }) => {
  const { modal, toggle, data, deleteTeacher } = props;
    ...
    <div>
      <Modal isOpen={modal} toggle={() => toggle(false)}>
        <Formik
          ...
          onSubmit={(values) =>
            deleteTeacher(values.id, alertSuccess, alertError)
          }
        >
          {() => (
            <Form>
...
const mapDispatchToProps = (dispatch) => {
  return {
    deleteTeacher: (id, onSuccess, onFailure) =>
      dispatch(deleteTeacher(id, onSuccess, onFailure)),
  };
};
